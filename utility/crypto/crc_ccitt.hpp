...
#ifndef __UTILITY_CRYPTO_CCITT__
#define __UTILITY_CRYPTO_CCITT__
...
namespace utility
{
  namespace crypto
  {
    namespace CRC_table
    {
      constexpr const unsigned short CCITT_TABLE[256] =
      {
    ...
    unsigned short CCR_CCITT(unsigned short __crc, unsigned short __ch) noexcept
    {
      __ch <<= 8;
      for(unsigned short __i = 0; __i < 8; ++__i)
      {
        if((__ch ^ __crc) & 0x8000U)
        { __crc = (__crc << 1) ^ 0x1021U;}
        else
        { __crc <<= 1;}
        __ch <<= 1;
      }
...
#endif // ! __UTILITY_CRYPTO_CCITT__
